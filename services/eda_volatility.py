...
def analyze_volatility_clusters(rates, pair_name="USD/NGN", window=5):
    ...
    analysis = {
        ...
        "clustering_confirmed": any(
            ac_abs.get(lag, 0.0) > ac_raw.get(lag, 0.0) for lag in range(1, 4)
        ),
    }
    return analysis
...
def run_eda_cli():
    ...
    if rate_rows:
        ...
        print(
            "No rates found in database. Using simulated rate sequence to demonstrate clustering..."
        )
        ...
        analysis = analyze_volatility_clusters(
            sim_rates, pair_name="USD/NGN (Simulated)"
        )
